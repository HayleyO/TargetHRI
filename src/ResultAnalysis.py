...
if __name__ == "__main__":
    ...
    if TRAIN:
        ...
        for run in range(RUNS):
            print("Run " + str(run) + ":")
            rl = Q_Learning_RL_environment(episodes=1, epsilon=EPSILON)
        ...
        for run in range(RUNS):
            print("Run " + str(run) + ":")
            rl = Q_Learning_RL_environment(episodes=1, epsilon=EPSILON, trust_alg=False)
            rl.load_q_table(name="Test_Err")
            #rl.load_epsilon(name="Test_Err")
            rewards_per_episode = rl.run_episodes(print_grid=False, print_data=PRINT_DATA, train=False)
        ...
        for run in range(RUNS):
            print("Run " + str(run) + ":")
            rl = Q_Learning_RL_environment(episodes=1, epsilon=EPSILON, trust_alg=False, guidance=False)
            rl.load_q_table(name="Test_Err")
            #rl.load_epsilon(name="Test_Err")
            rewards_per_episode = rl.run_episodes(print_grid=False, print_data=PRINT_DATA, train=False)
